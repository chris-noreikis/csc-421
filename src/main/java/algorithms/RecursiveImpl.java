...
public class RecursiveImpl extends EditDistanceAlgorithm {
    ...
    public int solveRecursive(String initial, String target, int inIndex, int tarIndex) {
        ...
        return Math.min(
                Math.min(
                        solveRecursive(initial, target, inIndex, tarIndex - 1) + 1,
                        solveRecursive(initial, target, inIndex - 1, tarIndex) + 1),
                solveRecursive(initial, target, inIndex - 1, tarIndex - 1) + replaceCount
        );
    }
}
